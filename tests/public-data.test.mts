...
test("todos los registros maestros quedan materializados", () => {
  ...
  assert.equal(rows.claims.length, 1256);
  assert.equal(rows.evidence.length, 1063);
  assert.equal(rows.sources.length, 1500);
  assert.equal(rows.controversies.length, 565);
  assert.equal(rows.errors.length, 678);
  assert.equal(rows.timeline.length, 639);
});
