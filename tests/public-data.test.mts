...
test("todos los registros maestros quedan materializados", () => {
  ...
  assert.equal(rows.claims.length, 1354);
  assert.equal(rows.evidence.length, 1117);
  assert.equal(rows.sources.length, 1557);
  assert.equal(rows.controversies.length, 589);
  assert.equal(rows.errors.length, 706);
  assert.equal(rows.timeline.length, 676);
});
