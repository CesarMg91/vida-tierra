...
test("todos los registros maestros quedan materializados", () => {
  ...
  assert.equal(rows.claims.length, 1303);
  assert.equal(rows.evidence.length, 1091);
  assert.equal(rows.sources.length, 1534);
  assert.equal(rows.controversies.length, 577);
  assert.equal(rows.errors.length, 692);
  assert.equal(rows.timeline.length, 659);
});
