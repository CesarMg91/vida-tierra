...
test("todos los registros maestros quedan materializados", () => {
  ...
  assert.equal(rows.claims.length, 1405);
  assert.equal(rows.evidence.length, 1140);
  assert.equal(rows.sources.length, 1585);
  assert.equal(rows.controversies.length, 599);
  assert.equal(rows.errors.length, 718);
  assert.equal(rows.timeline.length, 691);
});
