...
import type { AmericasMedicineArchiveData, AmericasMedicineLayerKind } from "../lib/americas-medicine-archives";

const labels: Record<AmericasMedicineLayerKind, string> = {
  archive: "Archivo",
...
export default function AmericasMedicineArchiveLab({ data }: { data: AmericasMedicineArchiveData }) {
  ...
  return (
    <div className="clinical-lab documentary-medicine-lab americas-medicine-archive-lab">
      <div className="clinical-toolbar">
        <div>
          <label htmlFor={selectId}>Archivo americano auditado</label>
      ...
      <footer className="clinical-lab-footer">
        <div>
          <strong>Una huella no hereda una historia clínica.</strong>
          <p>Cuerpo, residuo y códice responden preguntas distintas. La cadena conserva procedencia, alternativa y límite antes de hablar de práctica o efecto.</p>
        </div>
        <Link className="button button-primary" href={`/${data.researchSlug}`}>Abrir MED-010 <ArrowRight aria-hidden="true" /></Link>
      </footer>
    </div>
  );
}
