...
import type { EpidemicsQuarantineEvidenceData, EpidemicsQuarantineLayerKind } from "../lib/epidemics-quarantine-evidence";

const labels: Record<EpidemicsQuarantineLayerKind, string> = {
...
export default function EpidemicsQuarantineEvidenceLab({ data }: { data: EpidemicsQuarantineEvidenceData }) {
  ...
  if (!selected) {
    return (
      <div className="clinical-empty" role="status">
        ...
        <p>No hay un expediente cargado. Es un estado vacío, no evidencia de ausencia de epidemias o respuestas.</p>
      </div>
    );
  }

  return (
    <div className="clinical-lab documentary-medicine-lab epidemics-quarantine-evidence-lab">
      <div className="clinical-toolbar">
        <div>
          <label htmlFor={selectId}>Epidemia o respuesta auditada</label>
      ...
      <ol className="clinical-chain" aria-label={`Cadena desde testimonio hasta desigualdad para ${selected.title}`}>
      ...
      <footer className="clinical-lab-footer">
        <div>
          <strong>Un brote narrado no es un diagnóstico ni una cifra.</strong>
          <p>Testimonio, agente, magnitud, respuesta, cumplimiento y desigualdad permanecen separados antes de atribuir causa, efecto o justicia.</p>
        </div>
        <Link className="button button-primary" href={`/${data.researchSlug}`}>Abrir MED-014 <ArrowRight aria-hidden="true" /></Link>
      </footer>
    </div>
  );
}
