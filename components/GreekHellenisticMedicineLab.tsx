...
import type {
  GreekHellenisticMedicineEvidenceData,
  GreekHellenisticMedicineLayerKind,
} from "../lib/greek-hellenistic-medicine-evidence";

const labels: Record<GreekHellenisticMedicineLayerKind, string> = {
  witness: "Testimonio",
  reading: "Lectura",
  genre: "Género",
  operation: "Operación",
...
export default function GreekHellenisticMedicineLab({ data }: { data: GreekHellenisticMedicineEvidenceData }) {
  ...
  return (
    <div className="clinical-lab documentary-medicine-lab">
      <div className="clinical-toolbar">
        <div>
          <label htmlFor={selectId}>Archivo del Mediterráneo griego y helenístico</label>
      ...
      <ol className="clinical-chain" aria-label={`Cadena desde testimonio hasta consecuencia para ${selected.title}`}>
      ...
      <footer className="clinical-lab-footer">
        <div>
          <strong>Observación escrita no es resultado clínico.</strong>
          <p>La cadena separa testimonio, lectura, género, operación, institución y consecuencia para no heredar autoría, práctica o eficacia.</p>
        </div>
        <Link className="button button-primary" href={`/${data.researchSlug}`}>Abrir MED-007 <ArrowRight aria-hidden="true" /></Link>
      </footer>
    </div>
  );
}
