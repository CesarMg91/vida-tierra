...
import type { IndianOceanEurasianMedicineLayerKind, IndianOceanEurasianMedicineRouteData } from "../lib/indian-ocean-eurasian-medicine-routes";

const labels: Record<IndianOceanEurasianMedicineLayerKind, string> = {
  trace: "Huella",
...
export default function IndianOceanEurasianMedicineRouteLab({ data }: { data: IndianOceanEurasianMedicineRouteData }) {
  ...
  if (!selected) {
    return (
      <div className="clinical-empty" role="status">
        ...
        <p>No hay una ruta cargada. Es un estado vacío, no evidencia de aislamiento histórico.</p>
      </div>
    );
  }

  return (
    <div className="clinical-lab documentary-medicine-lab indian-ocean-eurasian-medicine-route-lab">
      <div className="clinical-toolbar">
        <div>
          <label htmlFor={selectId}>Ruta o archivo auditado</label>
      ...
      <ol className="clinical-chain" aria-label={`Cadena desde huella hasta alcance para ${selected.title}`}>
      ...
      <footer className="clinical-lab-footer">
        <div>
          <strong>Una ruta no demuestra una transferencia.</strong>
          <p>Huella, identificación, itinerario, mediación, recepción y alcance permanecen separados antes de atribuir cambio o efecto.</p>
        </div>
        <Link className="button button-primary" href={`/${data.researchSlug}`}>Abrir MED-013 <ArrowRight aria-hidden="true" /></Link>
      </footer>
    </div>
  );
}
