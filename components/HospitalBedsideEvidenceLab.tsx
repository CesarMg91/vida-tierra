...
import type { HospitalBedsideEvidenceData, HospitalBedsideLayerKind } from "../lib/hospital-bedside-evidence";

const labels: Record<HospitalBedsideLayerKind, string> = {
...
export default function HospitalBedsideEvidenceLab({ data }: { data: HospitalBedsideEvidenceData }) {
  ...
  if (!selected) {
    return (
      <div className="clinical-empty" role="status">
        ...
        <p>No hay un expediente cargado. Es un estado vacío, no evidencia de ausencia de hospitales, pacientes, encuentros, registros o decisiones.</p>
      </div>
    );
  }

  return (
    <div className="clinical-lab documentary-medicine-lab hospital-bedside-evidence-lab">
      <div className="clinical-toolbar">
        <div>
          <label htmlFor={selectId}>Expediente hospitalario auditado</label>
      ...
      <ol className="clinical-chain" aria-label={`Cadena desde institución hasta decisión para ${selected.title}`}>
      ...
      <footer className="clinical-lab-footer">
        <div>
          <strong>Una cama hospitalaria no convierte a una persona en un caso por sí sola.</strong>
          <p>Institución, admisión, encuentro, examen, registro, comparación y decisión permanecen separados antes de atribuir conocimiento, eficacia o progreso.</p>
        </div>
        <Link className="button button-primary" href={`/${data.researchSlug}`}>Abrir MED-019 <ArrowRight aria-hidden="true" /></Link>
      </footer>
    </div>
  );
}
