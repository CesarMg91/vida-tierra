...
import type { ColonialEncountersEvidenceData, ColonialEncounterLayerKind } from "../lib/colonial-encounters-evidence";

const labels: Record<ColonialEncounterLayerKind, string> = {
  exposure: "Exposición",
  violence: "Violencia",
  labor: "Trabajo",
  nutrition: "Nutrición",
...
export default function ColonialEncountersEvidenceLab({ data }: { data: ColonialEncountersEvidenceData }) {
  ...
  if (!selected) {
    return (
      <div className="clinical-empty" role="status">
        ...
        <p>No hay un expediente cargado. Es un estado vacío, no evidencia de ausencia de violencia, cuidado o supervivencia.</p>
      </div>
    );
  }

  return (
    <div className="clinical-lab documentary-medicine-lab colonial-encounters-evidence-lab">
      <div className="clinical-toolbar">
        <div>
          <label htmlFor={selectId}>Encuentro colonial auditado</label>
      ...
      <ol className="clinical-chain" aria-label={`Cadena desde exposición hasta supervivencia para ${selected.title}`}>
      ...
      <footer className="clinical-lab-footer">
        <div>
          <strong>Contacto no es causa suficiente.</strong>
          <p>Exposición, violencia, trabajo, nutrición, clasificación, respuesta y supervivencia permanecen separadas antes de atribuir catástrofe, cuidado o continuidad.</p>
        </div>
        <Link className="button button-primary" href={`/${data.researchSlug}`}>Abrir MED-015 <ArrowRight aria-hidden="true" /></Link>
      </footer>
    </div>
  );
}
