...
import type { CirculationPhysiologyEvidenceData, CirculationPhysiologyLayerKind } from "../lib/circulation-physiology-evidence";

const labels: Record<CirculationPhysiologyLayerKind, string> = {
  system: "Sistema heredado",
...
export default function CirculationPhysiologyEvidenceLab({ data }: { data: CirculationPhysiologyEvidenceData }) {
  ...
  if (!selected) {
    return (
      <div className="clinical-empty" role="status">
        ...
        <p>No hay un expediente cargado. Es un estado vacío, no evidencia de ausencia de sistemas, experimentos, mediciones o recepción.</p>
      </div>
    );
  }

  return (
    <div className="clinical-lab documentary-medicine-lab circulation-physiology-evidence-lab">
      <div className="clinical-toolbar">
        <div>
          <label htmlFor={selectId}>Expediente fisiológico auditado</label>
      ...
      <ol className="clinical-chain" aria-label={`Cadena desde sistema heredado hasta recepción para ${selected.title}`}>
      ...
      <footer className="clinical-lab-footer">
        <div>
          <strong>Una ligadura no descubre una circulación por sí sola.</strong>
          <p>Sistema, preparación, intervención, medición, comparación, mecanismo y recepción permanecen separados antes de atribuir una transformación fisiológica.</p>
        </div>
        <Link className="button button-primary" href={`/${data.researchSlug}`}>Abrir MED-017 <ArrowRight aria-hidden="true" /></Link>
      </footer>
    </div>
  );
}
